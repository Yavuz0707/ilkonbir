...
export default function MarketValueChart({ points = [], title = "Piyasa Değeri" }) {
  ...
  if (data.length < 2) {
    return (
      <section className="app-panel p-5">
        ...
        <p className="mt-4 text-sm text-ink-muted">Trend verisi henüz yok.</p>
      </section>
    );
  }
  ...
  return (
    <section className="app-panel p-5">
      <div className="mb-4 flex items-end justify-between gap-3">
        <div>
          <p className="eyebrow">{title}</p>
          <h2 className="mt-1 font-display text-2xl font-black uppercase text-ink">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="h-56 w-full overflow-visible">
        ...
        {coords.map((point) => (
          <g key={`${point.date}-${point.value}`}>
            <circle cx={point.x} cy={point.y} r="5" fill="var(--color-night)" stroke="var(--accent)" strokeWidth="3" />
