...
export const api = {
  ...
  records: (params = {}) => request(`/records?${qs(params)}`),
  recordCategories: () => request("/records/categories"),
  recordDetail: (id) => request(`/records/${id}`),
  recordGroup: (groupId) => request(`/records/group/${groupId}`),
  recordSearch: (q) => request(`/records/search?${qs({ q })}`),
  computedRecords: () => request("/records/ilkonbir/computed"),
